...
async fn search_inner(mut ws: WebSocket, state: MioState, userid: Uuid) {
    // idea: dynamic keystroke sending and cancelling for db searching each
    // keystroke/string update is sent to here over the websocket. drop the future
    // being polled on such an update and spin a new future
    //
