...
impl MioFrontendWeak {
    ...
    async fn album_poll_task(self) {
        loop {
            ...
            drop(
                self.app
                    .upgrade_in_event_loop(|app| app.global::<AlbumsCB>().set_albums_setup(true)),
            );

            // update the image buffer when ready 
            for task in cover_art {
                if let Some((id, ret)) = task.await.unwrap() {
                    let pos = uuid_pos[&id];
